...
export const achive = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    const recursiveArchive = async (documentsId: Id<"documents">) => {
      const children = await ctx.db
        .query("documents")
        .withIndex("by_user_parent", (q) =>
          q.eq("userId", userId).eq("parentDocument", documentsId)
        )
        .collect();
...
export const restore = mutation({
  ...
  handler: async (ctx, args_0) => {
    ...
    const documets = await ctx.db.patch(args_0.id, options);
...
export const remove = mutation({
  ...
  handler: async (ctx, args_0) => {
    ...
    const deleted = await ctx.db.delete(args_0.id);
...
export const getById = query({
  ...
  handler: async (ctx, args_0) => {
    ...
    const document = await ctx.db.get(args_0.documentId);
